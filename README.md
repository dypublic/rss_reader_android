...
订阅页右上角进入设置可调整全局阅读字号，也可以手动检查 GitHub Releases 更新。正式版每天最多自动检查一次；发现新版后可在 App 内下载，系统会在校验 SHA-256、应用 ID、版本号和正式签名通过后要求用户确认安装。文章页右上角可再次调字号，点击图片可放大，原文按钮交给系统浏览器。
